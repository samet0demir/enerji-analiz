...
exports.getMigrationStatus = exports.runMigrations = void 0;
...
const runMigrations = async () => {
    ...
    try {
        ...
        const appliedMigrations = await client.query('SELECT id FROM migrations ORDER BY applied_at');
        ...
        for (const migration of migrations) {
            if (!appliedIds.includes(migration.id)) {
                ...
                try {
                    ...
                    await client.query('INSERT INTO migrations (id, description) VALUES ($1, $2)', [migration.id, migration.description]);
...
const getMigrationStatus = async () => {
    const pool = (0, database_1.getDbPool)();
    const result = await pool.query('SELECT * FROM migrations ORDER BY applied_at DESC');
    return result.rows;
};
